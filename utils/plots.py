...
def comparative_boxplot(results, labels, path):
    plt.figure(figsize=(8,6))
    # Use 'tick_labels' (new name) to avoid deprecation warnings in newer Matplotlib
...
def sensitivity_heatmap(matrix, xlabels, ylabels, path):
    plt.figure(figsize=(8,6))
    sns.heatmap(matrix, xticklabels=xlabels, yticklabels=ylabels, annot=True, cmap='viridis')
